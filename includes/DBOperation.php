...
class DBOperation
{
	...
	public function getCat()
	{
		...
		public function getCat4edit($id)
	{
	    $pre_stmt3 = $this->con->prepare("SELECT * FROM `medicine` WHERE pid = ?");
		$pre_stmt3->bind_param('s', $id);
		$pre_stmt3->execute() or die($this->con->error);
		$result3 = $pre_stmt3->get_result();
		...
		$pre_stmt = $this->con->prepare("SELECT * FROM `categories` WHERE cat_id = ?");
		$pre_stmt->bind_param('s', $q);
		$pre_stmt->execute() or die($this->con->error);
		$result = $pre_stmt->get_result();
		$pre_stmt2 = $this->con->prepare("SELECT * FROM `categories` WHERE cat_id != ?");
		$pre_stmt2->bind_param('s', $q);
		$pre_stmt2->execute() or die($this->con->error);
		$result2 = $pre_stmt2->get_result();
		if ($result->num_rows > 0) {
		...
		if($result2->num_rows > 0){
		    while ($row2 = $result2->fetch_assoc()) {
				...
				$cat .= "<option value='" . $cat_id2 . "'>" . $cat_name2 . "</option>";
			}
		}
			return $cat;
	}
